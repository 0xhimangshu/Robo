...
This is an entrypoint that sets up a basic Bot with Robo.
...
It can be used to perform manual administrative actions as the bot, or to test Robo itself.
...
async def entry(bot: commands.Bot, *args: typing.Any, **kwargs: typing.Any):
    ...
    await discord.utils.maybe_coroutine(bot.load_extension, 'robocog')  # type: ignore
...
def entrypoint(intents: typing.Iterable[str], token: str, log_level: str, log_file: typing.Optional[str] = None):
    """
    Entrypoint accessible through `python -m robocog <TOKEN>`

    Specify intents using + and - before the token
    E.g.:
        -m robocog -- +all -message_content <TOKEN>
    Arguments are applied in order.
    You can also set log level and output to a file:
        -m robocog --log-level INFO --log-file bot.log -- +all <TOKEN>
    """
    ...
    logging.getLogger('robocog.__main__').critical(
        'Generated a unique UUID for this session: %s'
        '\nYou can use Robo with your bot once it starts using `%s::jsk <subcommand>`'
    ...
    if discord.version_info >= (2, 0, 0):
        ...
        bot.load_extension('robocog')  # type: ignore
