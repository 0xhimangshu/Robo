...
from ..math import mean_stddev
from ..modules import ExtensionConverter
from ..repl import inspections
...
class ManagementFeature(Feature):
    ...
    @Feature.Command(parent="robo", name="load", aliases=["reload"])
    async def robo_load(self, ctx: ContextA, *extensions: ExtensionConverter):  # type: ignore
        """
        ...
        for extension in itertools.chain(*extensions):
            method, icon = (
                (self.bot.reload_extension, "<:greentick:1172524936292741131>")
                if extension in self.bot.extensions else
                (self.bot.load_extension, "<:greentick:1172524936292741131>")
            )
            ...
            try:
                ...
                paginator.add_line(f"{icon} loaded `{extension}`")

        for page in paginator.pages:
            await ctx.send(page)

    @Feature.Command(parent="robo", name="unload")
    async def robo_unload(self, ctx: ContextA, *extensions: ExtensionConverter):  # type: ignore
        """
    ...
    @Feature.Command(parent="robo")
    async def say(self, ctx: ContextA, *, message: str):
        """
        Says a message.
        """
    ...
    @Feature.Command(parent="robo", name="shutdown", aliases=["logout"])
    async def robo_shutdown(self, ctx: ContextA):
        """
        Logs this bot out.
        """
    ...
    @Feature.Command(parent="robo", name="invite")
    async def robo_invite(self, ctx: ContextA, *perms: str):
        """
    ...
    @Feature.Command(parent="robo", name="rtt", aliases=["ping"])
    async def robo_rtt(self, ctx: ContextA):
        """
    ...
    @Feature.Command(parent="robo", name="sync")
    async def robo_sync(self, ctx: ContextA, *targets: str):
        """
