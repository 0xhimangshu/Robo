...
from ..types import BotT, ContextA
...
class Feature(commands.Cog):
    ...
    async def cog_check(self, ctx: ContextA):  # type: ignore  # pylint: disable=invalid-overridden-method
        ...
        if not await ctx.bot.is_owner(ctx.author):
            raise commands.NotOwner("You must own this bot to use developer commands.")
        return True

    @contextlib.contextmanager
    def submit(self, ctx: ContextA):
        """
        A context-manager that submits the current task to robo's task list
