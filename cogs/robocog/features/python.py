...
from ..functools import AsyncSender
from ..math import format_stddev
from ..paginators import PaginatorInterface, WrappedPaginator, use_file_check
from ..repl import AsyncCodeExecutor, Scope, all_inspections, create_tree, disassemble, get_var_dict_from_ctx
...
class PythonFeature(Feature):
    ...
    @Feature.Command(parent="robo", name="retain")
    async def robo_retain(self, ctx: ContextA, *, toggle: bool = None):  # type: ignore
        """
    ...
    async def robo_python_result_handling(self, ctx: ContextA, result: typing.Any):  # pylint: disable=too-many-return-statements
        """
        Determines what is done with a result when it comes out of robo py.
    ...
    def robo_python_get_convertables(self, ctx: ContextA) -> typing.Tuple[typing.Dict[str, typing.Any], typing.Dict[str, str]]:
        """
    ...
    @Feature.Command(parent="robo", name="py", aliases=["python"])
    async def robo_python(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        """
        ...
        arg_dict, convertables = self.robo_python_get_convertables(ctx)
        scope = self.scope
            ...
            async with ReplResponseReactor(ctx.message):
                with self.submit(ctx):
                    ...
                    async for send, result in AsyncSender(executor):  # type: ignore
                        ...
                        send(await self.robo_python_result_handling(ctx, result))

        finally:
            scope.clear_intersection(arg_dict)

    @Feature.Command(parent="robo", name="py_inspect", aliases=["pyi", "python_inspect", "pythoninspect"])
    async def robo_python_inspect(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        """
    ...
    if line_profiler is not None:
        @Feature.Command(parent="robo", name="timeit")
        async def robo_timeit(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
            """
    ...
    @Feature.Command(parent="robo", name="dis", aliases=["disassemble"])
    async def robo_disassemble(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        """
    ...
    @Feature.Command(parent="robo", name="ast")
    async def robo_ast(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        """
