...
class Utility(commands.Cog):
    ...
    @commands.hybrid_command(usage="[reason] []")
    @commands.guild_only()
    @app_commands.describe(
        reason="The reason why you are AFK",
        option="Whether to set your AFK for guild or global or not (default: True)",
    )
    async def afk(
        self,
        ctx: Context,
        *,
        ...
        option: Optional[Literal["global", "guild"]] = None,
    ):
        """
        ...
        if reason is None:
            ...
            reason = reason.replace("@", "@\u200b").replace("&", "&\u200b").replace("guild", "").replace("global", "").replace("guild", "")
